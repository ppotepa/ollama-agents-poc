...
from src.core.prompt_interceptor import PromptType, InterceptionMode
...
try:  # LangChain optional
    ...
    print(f"✅ LangChain ChatOllama imported successfully for InterceptorAgent")
...
{{
    ...
    def _build_llm(self) -> Any:
        ...
        if not LANGCHAIN_AVAILABLE:
            print(f"⚠️  LangChain not available for phi3:mini LLM")
            return None
...
Provide 1-3 most relevant commands with confidence scores between 0.1 and 1.0."""
    ...
    def generate_lightweight_response(self, prompt: str) -> str:
        ...
        response_parts = [
            f"🔍 **Prompt Analysis Results**\n",
            f"**Top Command Recommendations:**\n"
        ]
    ...
    def process_query(self, query: str, **kwargs) -> str:
        ...
        if mode == InterceptionMode.LIGHTWEIGHT:
            ...
            response_parts = [
                f"🔍 **Detailed Prompt Analysis**\n",
                f"**Query:** {query}\n\n",
                f"**Command Recommendations:**\n"
            ]

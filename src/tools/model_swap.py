...
def suggest_better_model(current_task: str, current_model: str) -> str:
    ...
    # Analysis tasks
    if any(keyword in task_lower for keyword in ['analyze', 'analysis', 'examine', 'review', 'evaluate']):
        if 'qwen' not in current_model.lower():
            suggestions.append("qwen2.5:7b-instruct-q4_K_M - Strong analytical capabilities")
    
    # File operations
    if any(keyword in task_lower for keyword in ['file', 'directory', 'folder', 'list', 'read', 'write']):
        if 'deepcoder' not in current_model.lower():

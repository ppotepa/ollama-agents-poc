...
from src.core.intelligent_orchestrator import ExecutionStep, IntelligentOrchestrator, OrchestrationSession
...
def use_improved_strategies(strategy_manager: InvestigationStrategyManager) -> None:
    ...
    try:
        # Attempt to replace strategies with improved versions.  Some
        # implementations may remain abstract (missing required methods), in
        # which case instantiation will raise a ``TypeError``.  Catch and
        # log these errors without breaking the system.
        ...
        logger.warning(f"Could not apply improved strategies: {exc}. Falling back to defaults.")
        # Do not modify existing strategies; leave the defaults intact

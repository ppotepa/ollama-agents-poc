...
class SingleQueryStrategy(BaseStrategy):
    ...
    def _run_intelligent_investigation(self, query: str, streaming: bool = True) -> str:
        ...
        try:
            # Detect if an event loop is already running.  Intelligent
            # investigation relies on launching asynchronous tasks; if we are
            # already inside a running loop (e.g. called from another asyncio
            # context) it is unsafe to call ``asyncio.run``.  In that case
            # immediately fallback to the collaborative strategy.
            try:
            ...
            if running_loop and running_loop.is_running():
                self.logger.warning(
                    "Cannot run intelligent investigation inside existing event loop; "
                    "falling back to collaborative mode"
                )
            ...
            try:
                ...
                # If we are already inside a running event loop (e.g. nested asyncio.run),
                # fallback to collaborative mode instead of attempting to nest event loops.
                self.logger.warning(f"Cannot start nested event loop for intelligent investigation: {runtime_exc}")
                return self._fallback_collaborative_query(query, streaming)

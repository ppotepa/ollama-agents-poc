...
# Import investigation types from the legacy interface.  We attempt to import
# ``TaskAnalyzer`` from the same module for backward compatibility.  If it
# doesn't exist (as was the case in some earlier versions), we fall back
# to the stub defined in ``src.core``.
...
try:
    # TaskAnalyzer may not exist in the legacy module; try import
    from src.core.investigation_strategies import TaskAnalyzer  # type: ignore[attr-defined]
except Exception:
    # Fallback to stub provided in src.core package (see src/core/investigation_strategies.py)
    from src.core import TaskAnalyzer  # type: ignore
...
class ImprovedTaskAnalyzer(TaskAnalyzer):
    ...
    def _identify_subtasks(self, query: str) -> list[str]:
        ...
        if not subtasks:
            ...
            for indicator in step_indicators:
                matches = re.findall(rf"{indicator}[,:]?\s*([^.!?]*[.!?])", query, re.IGNORECASE)
    ...
    def _requires_tools(self, query: str, task_type: TaskType) -> bool:
        ...
        query = query.lower()
        return any(indicator in query for indicator in tool_indicators)
...
# Import base class for investigation strategies.  The legacy module defines
# ``BaseInvestigationStrategy`` which provides the required initialisation
# signature.  We explicitly inherit from this base class rather than the
# ``InvestigationStrategy`` enum to avoid ``enum`` inheritance errors.
from src.core.investigation import BaseInvestigationStrategy


class ImprovedInvestigationStrategy(BaseInvestigationStrategy):

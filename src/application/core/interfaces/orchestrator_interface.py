...
# orchestrator implementations (e.g. the strategy orchestrator) still
# reference it.  To avoid import errors and provide a minimal contract, we
# define a simple ``ExecutionSession`` here.  It tracks the session ID,
# the query being processed, optional mode and metadata, and the current
# status along with an optional result or error message.
...
class ExecutionSession:
    ...
    Attributes:
        ...
        status: Current status of the session ("initializing", "executing",
            "completed", "error", etc.).
